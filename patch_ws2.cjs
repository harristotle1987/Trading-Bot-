...
const regex = /          const wsUrl = "wss:\/\/stream\.bybit\.com\/v5\/public\/linear";[\s\S]*?ws = new WebSocket\(wsUrl\);[\s\S]*?ws\.onopen = \(\) => \{[\s\S]*?ws\.send\(JSON\.stringify\(\{[\s\S]*?\}\)\);[\s\S]*?\};[\s\S]*?ws\.onmessage = \(event\) => \{[\s\S]*?try \{[\s\S]*?const msg = JSON\.parse\(event\.data\);[\s\S]*?if \(!msg\.data\) return;[\s\S]*?const kline = msg\.data\[0\];[\s\S]*?candlestickSeriesInstance\.update\(\{[\s\S]*?\}\);[\s\S]*?\} catch \(e\) \{[\s\S]*?\}[\s\S]*?\};/g;
...
code = code.replace(/          const wsUrl = "wss:\/\/stream\.bybit\.com\/v5\/public\/linear";[\s\S]*?ws = new WebSocket\(wsUrl\);[\s\S]*?ws\.onopen = \(\) => \{[\s\S]*?ws\.send\(JSON\.stringify\(\{[\s\S]*?\}\)\);[\s\S]*?\};[\s\S]*?ws\.onmessage = \(event\) => \{[\s\S]*?try \{[\s\S]*?const msg = JSON\.parse\(event\.data\);[\s\S]*?[\s\S]*?candlestickSeriesInstance\.update\(\{[\s\S]*?\}\);[\s\S]*?\} catch \(e\) \{[\s\S]*?\}[\s\S]*?\};/, replacement.trim());
